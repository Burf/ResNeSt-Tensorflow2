...
def resnest_block(x, n_filter, stride_size = 1, dilation = 1, group_size = 1, radix = 1, block_width = 64, avd = False, avd_first = False, downsample = None, dropout_rate = 0., expansion = 4, is_first = False, stage = 1, index = 1):
    ...
    out = tf.keras.layers.BatchNormalization(axis = -1, epsilon = 1.001e-5, scale = True, name = "stage{0}_block{1}_bn1".format(stage, index))(out)
    if 0 < dropout_rate:
    ...
    if 0 < radix:
        ...
        out = tf.keras.layers.BatchNormalization(axis = -1, epsilon = 1.001e-5, scale = True, name = "stage{0}_block{1}_bn2".format(stage, index))(out)
        if 0 < dropout_rate:
    ...
    out = tf.keras.layers.BatchNormalization(axis = -1, epsilon = 1.001e-5, scale = True, name = "stage{0}_block{1}_bn3".format(stage, index))(out)
    if 0 < dropout_rate:
...
def resnest_module(x, n_filter, n_block, stride_size = 1, dilation = 1, group_size = 1, radix = 1, block_width = 64, avg_down = True, avd = False, avd_first = False, dropout_rate = 0., expansion = 4, is_first = True, stage = 1):
    ...
    if stride_size != 1 or tf.keras.backend.int_shape(x)[-1] != (n_filter * expansion):
        if avg_down:
            ...
            downsample = tf.keras.layers.BatchNormalization(axis = -1, epsilon = 1.001e-5, scale = True, name = "stage{0}_downsample_bn1".format(stage))(downsample)
        else:
...
def ResNet(x, stack, n_class = 1000, include_top = True, dilation = 1, group_size =1, radix = 1, block_width = 64, stem_width = 64, deep_stem = False, dilated = False, avg_down = False, avd = False, avd_first = False, dropout_rate = 0., expansion = 4):
    ...
    if deep_stem:
        ...
        out = tf.keras.layers.BatchNormalization(axis = -1, epsilon = 1.001e-5, scale = True, name = "stem_bn1")(out)
        ...
        out = tf.keras.layers.BatchNormalization(axis = -1, epsilon = 1.001e-5, scale = True, name = "stem_bn2")(out)
        ...
        out = tf.keras.layers.BatchNormalization(axis = -1, epsilon = 1.001e-5, scale = True, name = "stem_bn3")(out)
